...
function G:Get(path, parent, key)
	path = path or ""
	...
	-- Is this a file?
	local file = self.FS:GetFile(path)

	if (file) then
		local object = load_file(file, base)
		file:Close()

		object = object or true
		...
		local directory = self.FS:GetDirectory(path)
		...
		if (directory) then
			...
			if (object) then
				...
				local underscore = self:Get(module_join(path, "_"))

				if (underscore) then
					dictionary_shallow_copy(object, underscore)

					if (not getmetatable(underscore)) then
						setmetatable(underscore, getmetatable(object))
					end

					self._loaded[path] = underscore

					if (parent and key) then
						parent[key] = underscore
					end
